...
period_explain = "(Simulation Run dates: March 1, 2020 ~ {})".format(
    END_DATE.strftime("%B %d, %Y")
)
period_explain2 = "(Vaccination period: January 1, 2021 ~ {})".format(
    END_DATE.strftime("%B %d, %Y")
)
...
def time_all(dates, actualDF, infectedDF, hospitalizedDF, deaddDF):
    ...
    fig.add_trace(
        go.Scatter(
            x=dates,
            y=actualDF["hospitalizations"],
            name="Actual Hospitalization",
            line=dict(width=1.5, color="black"),
        ),
        row=2,
        col=1,
    ...
    # region helper lines
    fig.add_vline(
        x=datetime.strptime("2020-03-01", "%Y-%m-%d").timestamp() * 1000,
        line_width=1.5,
        line_dash="dot",
        line_color="red",
        annotation_text="1st COVID-19 case",
        annotation=dict(font_size=10, font_family="Times New Roman"),
        annotation_font_color="red",
        annotation_position="top right",
    )
    fig.add_vline(
        x=datetime.strptime("2020-12-14", "%Y-%m-%d").timestamp() * 1000,
        line_width=1.5,
        line_dash="dot",
        line_color="green",
        annotation_text="1st Vaccine administered ",
        annotation=dict(font_size=10, font_family="Times New Roman"),
        annotation_font_color="green",
        annotation_position="bottom right",
    )
    fig.add_vline(
        x=datetime.strptime("2021-05-01", "%Y-%m-%d").timestamp() * 1000,
        line_width=1.5,
        line_dash="dot",
        line_color="blue",
        annotation_text="1st Delta variants in simulation",
        annotation=dict(font_size=10, font_family="Times New Roman"),
        annotation_font_color="blue",
        annotation_position="top right",
    )
    fig.add_vline(
        x=datetime.strptime("2021-08-10", "%Y-%m-%d").timestamp() * 1000,
        line_width=1.5,
        line_dash="dot",
        line_color="orange",
        annotation_text="School open with mask-wearing recommendation ",
        annotation=dict(font_size=10, font_family="Times New Roman"),
        annotation_font_color="orange",
        annotation_position="bottom right",
    )
    fig.add_vline(
        x=datetime.strptime("2021-12-07", "%Y-%m-%d").timestamp() * 1000,
        line_width=1.5,
        line_dash="dot",
        line_color="purple",
        annotation_text="1st Omicron variant ",
        annotation=dict(font_size=10, font_family="Times New Roman"),
    ...
    fig.add_vline(
        # x=datetime.strptime("2023-02-15", "%Y-%m-%d").timestamp() * 1000,
...
def time_gender(dates, gender_dict):
    ...
    fig.update_yaxes(range=[0, 2000], col=1)
    fig.update_yaxes(range=[0, 750], col=2)
...
def time_age(dates, age_dict):
    ...
    # forecast - set y axis range to match dashboard figures
    fig.update_yaxes(range=[0, 750], col=1)
    fig.update_yaxes(range=[0, 500], col=2)
    fig.update_yaxes(range=[0, 50], col=3)
...
def time_race(dates, race_dict):
    ...
    fig.update_yaxes(range=[0, 750], row=1, col=2)
...
def time_ethinic(dates, ethnic_dict):
    ...
    # forecast - set y axis range to match dashboard figures
    fig.update_yaxes(range=[0, 2750], row=1, col=1)
    fig.update_yaxes(range=[0, 750], row=1, col=2)
...
def time_income(dates, income_dict):
    ...
    fig.update_yaxes(range=[0, 500], row=1, col=2)
    fig.update_yaxes(range=[0, 100], row=1, col=3)
...
def draw_risky_zipcodes():
    ...
    riskzips_df = pd.read_parquet(
        os.path.join(asset_path, "risky_zipcodes.parquet"),
    )
...
# @cache.memoize(timeout=CACHE_TIMEOUT)
def bar_zipcodes():
    plot_df = pd.read_parquet(os.path.join(asset_path, "risky_zipcodes_bar.parquet"))
...
def draw_infected_map():
    ...
    mapDF = mapDF[mapDF["date_str"] >= START_DATE]
...
def render_content(tab):
    ...
    # Forecast
        ...
        # redirect to forecast url
        return html.Div(
            [dcc.Location(id="url", href="http://abm.seircast.org:8051", refresh=True)]
        )
